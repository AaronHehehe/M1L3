...
async def on_message(message):
    ...
    if "https://" in message.content:
        # Pastikan bot punya izin ban
        if message.guild and message.guild.me.guild_permissions.ban_members:
            try:
                await message.guild.ban(
                    message.author,
                    reason="Mengirim link / iklan"
                )
                await message.channel.send(
                    f" {message.author.name} telah diblokir karena mengirim link."
                )
            except discord.Forbidden:
                await message.channel.send("Saya tidak memiliki izin untuk memblokir pengguna ini.")
